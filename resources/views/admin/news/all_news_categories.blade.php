...
@section('heading', 'News Categories')

@section('button_section')

    <div class="d-flex">
        <form action="{{ route('news_category_show') }}" method="GET" class="d-flex">
        ...
        <a href="{{ route('news_category_add') }}" class="btn btn-primary ms-2" data-bs-toggle="tooltip"
            data-bs-placement="top" title="Add New"><span class="ti ti-plus"></span></a>

        {{-- Tombol Kembali akan muncul jika ada pencarian --}}
        @if (request()->input('search'))
            <a href="{{ route('news_category_show') }}" class="btn btn-warning ms-2">Back <i
                                        ...
                                        @foreach ($all_data as $item)
                                            <tr>
                                                ...
                                                <td class="pt_10 pb_10">
                                                    <a href="{{ route('news_category_edit', $item->id) }}"
                                                        class="btn btn-primary">Edit</a>
                                                    <a href="{{ route('news_category_delete', $item->id) }}"
